...
exports.flashItemAlert = function (flashItemName = 'CT_FairyLights433Socket', numFlashes = 1, pulseTimeMs = 500, logger) {
  ...
  // Schedule all state changes including the first one
  for (let index = 0; index < numFlashes * 2; index++) {
    const state = (index % 2) === 0 ? 'ON' : 'OFF';
    const delay = index * pulseTimeMs;
  ...
  // Schedule restore of original state
  const finalDelay = numFlashes * 2 * pulseTimeMs;
