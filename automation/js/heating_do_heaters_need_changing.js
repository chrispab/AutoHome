...
rules.JSRule({
  ...
  execute: (event) => {
    ...
    logger.info(`>setpointItem: ${setpointItem.name}, state: ${setpointItem.state}`);

    const TemperatureItem = items.getItem(`${roomPrefix}_ThermostatTemperatureAmbient`);
    logger.info(`>TemperatureItem: ${TemperatureItem.name}, state: ${TemperatureItem.state}`);
    ...
    logger.info(`>ReachableItem: ${ReachableItem.name}, state: ${ReachableItem.state}`);
    ...
    logger.info(`>masterHeatingMode.state.toString() : ${items.getItem('masterHeatingMode').state.toString()}`);
    ...
    if (((heatingModeItem.state.toString() === 'auto')) || ((heatingModeItem.state.toString() === 'manual'))) {
      logger.info(`>>Heater: ${roomPrefix}, mode is: ${heatingModeItem.state.toString()}`);
