...
scriptLoaded = function () {
  logger.warn('scriptLoaded - pir 03 03');
  // loadedDate = Date.now();
};
...
rules.JSRule({
  ...
  description: 'pir03  - Turn ON lights, cancel off timers  ',
  triggers: [
...
rules.JSRule({
  name: 'PIRsensor 03ON to OFF - start the lights off timer',
  ...
  execute: (data) => {
    ...
    if (data.itemName === 'pir03_occupancy') {
      ...
      logger.warn('-pir03_occupancy: STARTING TIMER KT_light_1_Power: OFF END');
    }
  },
});
